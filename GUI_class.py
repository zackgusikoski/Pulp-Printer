...
class window(tkinter.Tk):
    def __init__(self):
        ...
        # add logo to window
        self.iconphoto(False, PhotoImage(file="test_image30.jpg"))
        ...
        Button(self.page, text="Apply Settings", command=self.run).grid(row=5, column=4)

        # save settings
        Button(self.page, text="Save Settings", command=self.save_settings).grid(row=5, column=5)


        # load settings
        Button(self.page, text="Load Settings", command=self.load_settings).grid(row=6, column=5)

        #visualization type selection
        Label(self.page, text="Visualization:").grid(row=5, column=2)
        self.visualization = StringVar(self.page)
        self.visualization.set(VISUALIZE[0])
        dropdown = OptionMenu(self.page, self.visualization, *VISUALIZE).grid(row=5, column=3)
    ...
    def image_selection(self):
        ...
        Label(self.self.page, text=image_name[-1]).grid(row=1, column=3)
    ...
    def run(self):
            ...
            FLOW_STEP = int(self.flowstep.get())
